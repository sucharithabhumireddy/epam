...
public class StudentGroup implements StudentArrayOperation {
	...
	public Student[] getByBirthDate(Date date) {
		try {
			int j=0;
			Student ResulStud[]=new Student[students.length];
			for(int i=0;i<students.length-1;i++)
			{
				if(students[i].getBirthDate().equals(date))
				{
					ResulStud[j++]=students[i];
				}
			}
			return ResulStud;
		}
	...
	public Student[] getStudentsWithMaxAvgMark() {
		try{
			...
			for(int i=0;i<students.length-1;i++)
			{
				if(students[i].getAvgMark()>=maxm)
				{
					maxm=students[i].getAvgMark();
				}
			}
			for(int i=0;i<students.length-1;i++)
			{
	...
	public Student getNextStudent(Student student) {
       try {
    	   int psit=0;
			...
			{
				if(students[m].equals(student))
				{
					psit=m;
					break;
				}
			}
			return students[psit++];
		}catch(IllegalArgumentException e)
		{
			e.printStackTrace();
		}
